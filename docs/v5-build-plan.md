...
**Renumbered 2026-08-27 (direct user decision).** This plan was written as v4 and is unchanged in substance — every decision, phase, DoD and traceability row below stands exactly as locked on 2026-08-26. What moved is its _position_: a UI/polish version (`docs/v4-build-plan.md`) now runs first, so accounts became v5, gamification/launch v6, and multiplayer v7. Reason recorded: every item on that list is client-only with no backend dependency and ships incrementally, and two of its items — the Settings rebuild and the app-feel/optimistic-rendering work — are surfaces this plan's 5.1 and 5.3 would otherwise build on top of and then have rebuilt underneath them. The companion implementation plan is `docs/superpowers/plans/2026-08-27-v5-accounts-implementation-plan.md` (T1–T13, renumbered with this file).
...
> **Amended 2026-08-31.** Read the amendment at the bottom of this file before acting on the table below. It confirms the Workers + D1 backend against a Supabase/Postgres challenge and adds a written exit trigger (S1–S4), sets the v5 security posture (2FA, password rules, token storage, authorization — including two requested items rejected on security grounds), and adds CodeRabbit and Strix to the pipeline. No phase or estimate changed. **Further corrected 2026-09-10** — three Task 0 spending/vendor assumptions (Cloudflare Workers Paid, Strix, GitHub secret scanning) were wrong when checked against current vendor docs; see the dated updates in sections B, C and C-bis below. Task 0's actual cost is $0.
...
6. **Verification task (added 2026-09-08, see the B1 update above):** before building the sign-in/sign-up surface against Clerk's `useSignIn`/`useSignUp` hooks, confirm on Clerk's current pricing page that hook-based custom auth flows carry no branding requirement — lock the approach only after that's confirmed.
...
> **Update 2026-09-08, direct user decision: 2FA is dropped from v5 scope.** B1 above (TOTP + backup codes) is no longer adopted. Clerk stays on the free **Hobby** plan for the whole of v5 — Phase 5.1 builds sign-in/sign-up against Clerk's `useSignIn`/`useSignUp` hooks rather than the prebuilt components, and Hobby's mandatory branding requirement only binds the prebuilt UI, so it does not apply to a hook-based custom flow. This removes the Clerk Pro line item the 2026-08-31 correction above introduced. **Verification task added to Phase 5.1**: before locking in the hook-based approach, confirm on Clerk's current pricing page that hook-based custom auth flows carry no branding requirement.
...
> **Corrected 2026-09-10:** Clerk lists **custom password requirements as a Pro-tier feature** — the dashboard control this paragraph assumed ("minimum length set") may not be available on the free Hobby plan, and **Pro is not to be bought for it**. Hobby's baseline is already NIST-shaped (minimum 8, maximum 64 characters) with no complexity rules, which satisfies this decision's actual intent without a purchase. Task 0 records whatever password-policy controls Hobby does expose (compromised-password rejection, if offered) rather than configuring a minimum length that may not exist to configure.
...
> **Corrected 2026-09-10:** Clerk's default lockout is **10 failed attempts, then a 1-hour lockout**, configurable under Protect → Rules — but the default changed from 100 to 10 on 2026-07-06, and existing instances kept whichever value they already had. Task 0's item is therefore **verify this specific instance's configured number and record it**, not "confirm Clerk's behaviour" against a number that may not match reality.
...
Note what this does _not_ forbid: the app's existing local-first play state stays in IndexedDB exactly as it is. The rule is about credentials, not data.
...
> **Update 2026-09-08, direct user decision: the CodeRabbit Pro seat is cut.** v5 review coverage relies on **Free** (PR summaries) plus the C3 unglamorous controls — `pnpm audit` in CI, Dependabot, and secret scanning — rather than a paid agentic-review seat. Re-evaluate a paid seat only if that stack proves insufficient in practice.

> **Corrected 2026-09-10** (`codoro-v5-task0-cost-facts`): the "Free means summaries" framing above is itself wrong, not just the Pro decision — CodeRabbit's **Free plan does not cover private repositories at all**; Free is open-source-only, and the cheapest tier that touches a private repo is **Essentials, $24/dev/month minimum**. `codoro-app/codoro` is private, so there was never a zero-cost CodeRabbit tier available to "enable immediately." CodeRabbit is cut **entirely** — not scaled back to Free — and nothing above this line (the "enable Free immediately" decision, its cost line, its plan-reality paragraph) is still accurate. v5 PR review is **Claude Code + `pnpm audit` + Dependabot + gitleaks**, full stop; no CodeRabbit at any tier.

**C2 — Strix: agentic penetration testing against the dev env.** Apache-2.0, open source ([usestrix/strix](https://github.com/usestrix/strix)); runs autonomous agents that exercise a _running_ target and validate findings with proof-of-concept exploits, covering the OWASP Top 10 (injection, XSS, broken access control, SSRF, CSRF, JWT attacks, race conditions, API flaws). It runs via CLI or a GitHub Actions workflow, needs Docker and an LLM provider API key, and can take a live URL or an OpenAPI spec as its target.

This is **DAST, not SAST** — it needs something deployed to attack — so it does **not** belong on per-PR CI. It has ~~two~~ **one** scheduled home:

1. ~~**End of Phase 5.0**, pointed at the dev env, targeting `POST /api/report` specifically. That is the only unauthenticated write in the entire system and therefore the one endpoint the 5.6 authz suite structurally cannot cover; an agent that actually throws malformed enums, oversized bodies and forged puzzle ids at a live instance is the right instrument for it.~~ **Cut 2026-09-10 — see the update below; Phase 5.0 uses ZAP instead.**
...
> **Corrected 2026-09-10: Strix's Phase 5.0 slot is cut, not just deferred.** Phase 5.0's entire external attack surface is `GET /api/health` and one unauthenticated `POST /api/report` whose body is a Zod-validated fixed enum plus a `puzzleId` checked against the real content index — an LLM-driven exploitation agent finds nothing there that T4a's own abuse-input tests do not already cover, and it bills LLM tokens per run for that nothing. Phase 5.0 uses **zaproxy/action-baseline** instead: free, Docker-only, no LLM API key, same target (`codoro-dev`, `/api/health` + `/api/report`). The binding rule above (never production, target never read from an input) applies to the ZAP workflow exactly as it applied to Strix. Strix itself moves to **Phase 5.6 only** — item 2 above is now its sole run, not "run #2".
...
- ~~Secret scanning with push protection — verify at T0 what is actually available for a private repo on the current GitHub plan; if it is gated, add gitleaks as a CI step instead, which is free and plan-independent.~~ **Corrected 2026-09-10, no check needed:** GitHub's secret scanning + push protection is free only for public repositories; a private repo (`codoro-app/codoro` is private) requires paid **GitHub Secret Protection**. This resolves directly to **gitleaks** as a CI step, added during T1 — free, plan-independent, no availability check to run first. A leaked `sk_live_` Clerk secret or a Resend key is the single highest-severity failure available to this project, and it is prevented by a pre-push check, not by a pentest.
...
| ---------------------------- | ------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~Cloudflare Workers Paid~~  | ~~$5/mo~~ → **$0 at Task 0**         | **Deferred 2026-09-10** — verified free-plan limits (D1: 500 MB/database, 10 databases, 5 GB account total, 5M rows read/day, 100k rows written/day; Workers: 100k requests/day, 10 ms CPU/invocation) don't bind during 5.0 (server-only, ~40 lifetime visitors). Upgrade is a plan toggle, no migration. Buy it when the **first** of: (a) 5.2's in-Worker gzip (S1) measured against the 10 ms CPU ceiling — CPU, not storage, is the real trigger; (b) prod D1 nearing 500 MB; (c) sustained traffic near 100k req/day |
| ~~Clerk Pro~~                | ~~~$20/mo annual (~$25 monthly)~~    | Cut 2026-09-08 — 2FA dropped from v5 scope, Clerk stays on Hobby (free); see the B1 update above                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ~~CodeRabbit Pro, one seat~~ | ~~~$24/mo~~                          | Cut 2026-09-08 — **corrected 2026-09-10: CodeRabbit is cut entirely, not scaled to Free** (Free doesn't cover private repos at all); Claude Code + `pnpm audit` + Dependabot + gitleaks instead; see the C1 update above                                                                                                                                                                                                                                                                                                   |
| Resend                       | $0 on the free tier, then ~$20/mo    | Phase 5.5 — free tier is ample pre-launch                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Strix                        | $0 (Apache-2.0) + LLM tokens per run | **5.6 only** — 5.0's slot is ZAP (`zaproxy/action-baseline`, $0, no LLM key) instead, see the C2 update                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
> **Update 2026-09-08:** with the Clerk Pro and CodeRabbit Pro line items both cut (above), v5's actual run rate is **≈ $5/month (Workers Paid) until Phase 5.5**, when Resend's free tier likely runs out (~$20/mo). No $30–75/month range applies unless 2FA or a paid review seat comes back into scope.

> **Update 2026-09-10:** with Cloudflare Workers Paid also deferred (above) and Strix's 5.0 slot replaced by the free ZAP scan, **Task 0 and Phase 5.0 cost $0/month**, not ≈$5. The run rate stays $0 until Phase 5.5, when Resend's free tier likely runs out (~$20/mo) — unless the Workers Paid trigger above fires first.
...
| ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| T0 (pre-code) | Clerk password policy configured and **recorded** (~~MFA config moves to 5.1 — it needs Pro, see B1~~ cut 2026-09-08, see the B1 and C1 updates above; ~~minimum length set in the dashboard~~ **corrected 2026-09-10** — that control may be Pro-only, record what Hobby actually exposes, see the B2 update); Clerk lockout behaviour ~~confirmed~~ **verified against this instance's actual configured value** (corrected 2026-09-10, see the B3 update); Development-instance keys only, Production moves to 5.1 (corrected 2026-09-10); ~~CodeRabbit Free enabled (~~and one Pro seat bought~~ cut 2026-09-08)~~ **CodeRabbit cut entirely, corrected 2026-09-10 — Free has no private-repo tier; PR review is Claude Code + `pnpm audit` + Dependabot + gitleaks**; Dependabot on; ~~secret-scanning availability checked (else gitleaks)~~ **gitleaks added at T1, no check needed — private repos require paid GitHub Secret Protection** (corrected 2026-09-10); ~~LLM key for Strix stored as an Actions secret~~ **cut 2026-09-10 — Strix moves to 5.6, Phase 5.0 uses free ZAP instead** (see the C2 update); ~~Cloudflare Workers Paid active~~ **deferred 2026-09-10, see the C-bis update** |
| 5.0           | S1 (compressed payload) · S2 (`profileStore` boundary) · S3 (`scores` / `scores_best` split in migration 0001) · I9 + I10 with tests · ~~Strix run #1~~ **ZAP baseline scan** against the dev env's report endpoint (corrected 2026-09-10, see the C2 update) · `pnpm audit` + gitleaks in CI                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 5.1           | ~~Clerk Pro seat bought here (MFA is not on Hobby, and Hobby forces Clerk branding on the prebuilt UIs); MFA configured to TOTP + backup codes; enrolment surfaced in the Settings account section~~ — cut 2026-09-08 (2FA dropped from v5, see B1 update above). Instead: sign-in/sign-up built against Clerk's `useSignIn`/`useSignUp` hooks on Hobby, with the branding-requirement verification task from 5.1's build list                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| 5.6           | ~~Strix run #2~~ **Strix's only run** across the full API (corrected 2026-09-10 — there is no run #1 anymore) · S4 size numbers in the load-test record, **plus CPU milliseconds per invocation alongside latency (T14, added 2026-09-10 — CPU is the documented Workers Paid upgrade trigger)** · CodeRabbit seat cost in the cost curve · the exit trigger reviewed against real numbers                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
