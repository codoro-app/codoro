...
- Mapped into Tailwind via `--duration-press`/`--duration-feedback`/`--duration-enter`/`--ease-out`/`--ease-in-out` in the `@theme inline` block, generating real `duration-press`/`duration-feedback`/`duration-enter`/`ease-out`/`ease-in-out` utility classes — the same "alias, not copy" pattern every other token namespace in that block already uses
