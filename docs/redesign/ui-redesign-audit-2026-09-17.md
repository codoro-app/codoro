...
- [ ] **Page transitions** — in scope for the motion pass, or left instant? Real engineering cost either way. Stayed instant through Phase 4 (routing untouched) — still an open call for the eventual motion pass, not a final answer.
...
- `FirstRunSequence.tsx` has no automated test coverage of its own rendered output at all (only its underlying hook, `useFirstRunSession.test.ts`, is tested) — the progress-indicator addition there rode on that gap rather than closing it; worth a real component test in a future pass, independent of this redesign.
