...
- [ ] Direct load of `getcodoro.com/puzzle/<real-id>` on production renders the right puzzle — **blocked, see amendment: production currently 308s every direct route load to `/`, unrelated to this phase's code**
...
4. **Item 2 — a known, carried-forward risk this surface directly re-exposes.** `OD-3` (backward-scrub masking leak, "Known open defects" table above) is not fixed by this phase and was never in its scope — but `/puzzle/:id` serves `puzzlePool` (the full union) with no filter, so `tc-009`'s two leaky checkpoints are now reachable by a **stranger** following a shared link, not just a rated Trace session. This doesn't change OD-3's status or owner (still undecided, still open), but it does raise the stakes on deciding it — flagging explicitly rather than letting a wider blast radius sit silently in a table nobody re-reads.
