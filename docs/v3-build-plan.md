...
- [ ] OD-1 root cause stated from captured on-device data, fix merged, and **Thomas's on-device re-test passes** (iPhone 15 Pro, PWA + tab — the bar every v2 fix skipped or failed). Defect-table row closed by commit, or re-waived with the capture attached — **root cause now stated from real capture (Amendment 3); fix merged 2026-08-10; on-device re-test of the OD-2 fix still owed.**
- [x] Revert-check test for the fix's testable mechanism, following v2's convention (`SwipeBinary.test.tsx`) — **met, 2026-08-10.** 25 tests; `preventDefault`-on-horizontal-resolve, the axis-tolerance, and (Amendment 3) the `window.scrollBy` forwarding all verified red when stashed out.
...
- [ ] `pnpm validate` green; instrumentation stripped or flag-gated — validate green (Amendment 3); instrumentation still active, intentionally — needed for the on-device capture pass, strips once OD-1 closes.
