...
- [ ] OD-1 root cause stated from captured on-device data, fix merged, and **Thomas's on-device re-test passes** (iPhone 15 Pro, PWA + tab — the bar every v2 fix skipped or failed). Defect-table row closed by commit, or re-waived with the capture attached — **two real root causes found and fixed from two rounds of on-device capture (Amendments 3–4); a third re-test still owed.**
- [x] Revert-check test for the fix's testable mechanism, following v2's convention (`SwipeBinary.test.tsx`) — **met, 2026-08-10.** 26 tests; `preventDefault`-on-horizontal-resolve, the axis-tolerance, the `window.scrollBy` forwarding, and (Amendment 4) the skipped touch `setPointerCapture` call all verified red when stashed out.
...
- [ ] `pnpm validate` green; instrumentation stripped or flag-gated — validate green (Amendment 4); instrumentation still active, intentionally — needed for the on-device capture pass, strips once OD-1 closes.
