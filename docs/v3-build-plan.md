...
- [ ] OD-1 root cause stated from captured on-device data, fix merged, and **Thomas's on-device re-test passes** (iPhone 15 Pro, PWA + tab — the bar every v2 fix skipped or failed). Defect-table row closed by commit, or re-waived with the capture attached — **three real root causes found and fixed from three rounds of on-device capture (Amendments 3–5); a fourth re-test still owed.**
- [x] Revert-check test for the fix's testable mechanism, following v2's convention (`SwipeBinary.test.tsx`) — **met, 2026-08-10.** 26 tests; `preventDefault`-on-horizontal-resolve, the axis-tolerance, the `window.scrollBy` forwarding, the skipped touch `setPointerCapture` call, and (Amendment 5) the debug overlay's rAF-batched flush all verified red when stashed out or checked directly.
...
- [ ] `pnpm validate` green; instrumentation stripped or flag-gated — validate green (Amendment 5); instrumentation still active, intentionally — needed for the on-device capture pass, strips once OD-1 closes.
