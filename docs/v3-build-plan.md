...
- [ ] OD-1 root cause stated from captured on-device data, fix merged, and **Thomas's on-device re-test passes** (iPhone 15 Pro, PWA + tab — the bar every v2 fix skipped or failed). Defect-table row closed by commit, or re-waived with the capture attached — **four rounds of on-device capture (Amendments 3–6): three real root causes fixed, one candidate disproven by evidence, and a full architecture pivot (OD-5) informed by a proven reference implementation. A fifth re-test is the remaining step.**
