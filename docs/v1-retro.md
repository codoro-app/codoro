...
All four measure adoption or longevity that isn't coming. What v1's reduced Phase 9 pass _did_ verify instead — `pnpm validate` from a fresh clone, Lighthouse against production, 404/favicon/OG tag coverage, the error boundary → PostHog wire, an export/clear/import round-trip, and a real service-worker update check — is recorded in the build plan's Phase 9 checklist.
