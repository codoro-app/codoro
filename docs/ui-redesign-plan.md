...
**Tooling note**: this is a genuinely new composition problem (no existing precedent screen to follow), so `ui-ux-pro-max` may be used here — **constrained**: feed it 2b.1's token file and `docs/design/codoro-v2-arena.html` directly, and instruct it not to generate or suggest a new palette/font pairing. Use it for layout/hierarchy/UX-pattern guidance only. Do not let it run its default "generate a design system" flow — the visual identity is already decided.
...
**Tooling note**: same constrained use of `ui-ux-pro-max` as 2b.5 — another screen with no existing precedent in the app. Feed it the token file + arena reference, layout/composition guidance only, no new palette/font generation.
...
## `ui-ux-pro-max` — install (used only in 2b.5/2b.7, see those sections for constraints)

```bash
# prereq: Python 3.x on PATH
python3 --version
...
No global install needed — `npx` is enough for a one-repo skill. Update later with `uipro update` if a global install was used instead, or just re-run the `npx init` command to refresh.
