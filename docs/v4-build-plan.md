...
| Skeletons / caching / optimistic rendering (todo 9, 10, 11) | **Not built here.** Deferred to v5, where a network first exists                                                                                                                                                                                                                      | `src/app/RouteSkeleton.tsx`'s own doc comment already settled this in v2: Codoro is local-first — puzzles ship in the bundle, profile/attempt reads resolve off IndexedDB in single-digit milliseconds. A skeleton over a synchronous read makes the app feel _slower_; there is no network response to cache and nothing to render optimistically. The one genuine loading boundary (the lazy route chunk) already has its skeleton. These three items become real work the day sync and leaderboards exist — v5's 5.2/5.3 — and the v5 plan already claims item 11. Building them now would mean inventing fake latency to decorate.  |
| Difficulty filter (todo 17)                                 | **Skipped for v4** (direct user decision, 2026-08-28) — see Phase 4.2 below                                                                                                                                                                                                           | Building this properly would have required a genuinely new surface — a filterable list of individual puzzles to browse (today's `/browse` is a pattern picker, not a puzzle list, and it feeds the rated flow's pool, which the filter is explicitly forbidden from touching). Thomas does not want players seeing a list of puzzles at all, which removes the only shape this filter could safely take without either building a puzzle-list UI he doesn't want or leaking difficulty into rated selection. Thomas's own instinct on this item at plan time was "not sure if that is a great one" — on reaching it, the answer was no. |
...
| 4.2   | ~~Browse difficulty filter~~ — **skipped** (2026-08-28): would have required a puzzle-list browse UI Thomas doesn't want        | 0             |
...
## Phase 4.2 — Browse difficulty filter — **SKIPPED** (direct user decision, 2026-08-28)

**Not built.** Investigating this phase before writing its implementation plan surfaced a real architecture problem the build plan itself hadn't caught: today's `/browse` route is `PatternPicker.tsx`, a picker over the 8 bug **patterns** (not individual puzzles) whose selection feeds `usePracticeSession.setPatternFilter` — i.e. it narrows the pool for the **rated** Elo-selected Practice flow. There is no existing surface that lists individual puzzles. Making a difficulty filter mean anything (and satisfying this phase's own DoD language — "a band × pattern × mastery combination that matches nothing must render a named empty state, not a blank list" — only parses against a list of individual puzzles) would have required building a new, filterable puzzle-list browsing UI, since the locked decision above forbids the alternative (feeding difficulty into the rated flow the way pattern already is).

Asked directly: **Thomas does not want players seeing a list of puzzles**, full stop. That removes the only shape this filter could safely take. Todo item 17 is therefore closed as **not built, by decision** rather than carried forward — there is no smaller version of this that both respects "never the rated flow" and avoids a puzzle list.
...
3. **"Make daily better" beyond the interaction mix** is explicitly _out of scope here_ and belongs to v6's game-feel definition session — the session shape, the stakes, the payoff of a Daily are exactly what that session exists to define. This phase changes what Daily asks you to do; v6 changes what Daily _is_.
...
| 17  | Tiered answers easy/medium/hard              | **Not built — skipped** (2026-08-28 direct user decision). The only viable shape required a puzzle-list Browse UI, which Thomas doesn't want. See Phase 4.2 |
...
| 21  | No swipe or mcq for Daily; make Daily better | **4.3** for the interaction mix + calendar rebuild (firm; gated on a content batch); **v6** for what a Daily _is_ (game-feel definition session)            |
